...
- **Alias prefix collision resolved:** git owns the `gc*` namespace (`gc`=`git commit`). gcloud moves to `gx*` prefix. Specific renames: `gc`→`gx`, `gcp`→`gxc`, `gcl`→`gxl`, `gca`→`gxa`, `gcr`→`gxr`, `gce`→`gxe`, `gke`→`gxk`, `gsq`→`gxs`. DESIGN.md and shell.md to be updated in a housekeeping commit after Plan 2 merges.
...
- **Linux `port` differs from `ports`:** shell.md has both as `ss -tlnp`; amended to `port='ss -anp'` (all connections) vs `ports='ss -tlnp'` (listening only), matching the macOS intent where `port` is broader than `ports`.
...
Verify the log line was preserved:
